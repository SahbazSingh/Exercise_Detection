...
class SquatDetection:
    def __init__(self) -> None:
        self.ML_MODEL_PATH = os.path.join(settings.BASE_DIR, 'ml_models', 'squat_model.pkl')
        self.INPUT_SCALER_PATH = os.path.join(settings.BASE_DIR, 'ml_models', 'squat_input_scaler.pkl')

        self.load_machine_learning_model()  # Load model 

        self.PREDICTION_PROB_THRESHOLD = 0.51
        self.VISIBILITY_THRESHOLD = 0.6
        self.FOOT_SHOULDER_RATIO_THRESHOLDS = [1.2, 2.8]
        self.KNEE_FOOT_RATIO_THRESHOLDS = {"up": [0.5, 1.0], "middle": [0.7, 1.0], "down": [0.7, 1.1]}
        ...
        # Initialize previous_stage attribute
        self.previous_stage = {"feet": "", "knee": ""}
    ...
    def init_important_landmarks(self) -> None:
        ...
        self.headers = ["label"]  # Label column for DataFrame
        for lm in self.important_landmarks:
            self.headers += [
                f"{lm.lower()}_x",
                f"{lm.lower()}_y",
                f"{lm.lower()}_z",
                f"{lm.lower()}_visibility",
            ]

    def load_machine_learning_model(self):
        """Load machine learning model (and input scaler if necessary)."""
        try:
            with open(self.ML_MODEL_PATH, 'rb') as model_file:
                self.model = pickle.load(model_file)
            with open(self.INPUT_SCALER_PATH, 'rb') as scaler_file:
                self.input_scaler = pickle.load(scaler_file)
        except FileNotFoundError as e:
            raise FileNotFoundError(f"Model file not found: {e.filename}")
        except Exception as e:
            raise Exception(f"Error loading machine learning model: {e}")
    ...
    def handle_detected_results(self, video_name: str) -> dict:
        ...
        for index, error in enumerate(self.results):
            ...
            try:
                ...
                frame_path = os.path.relpath(frame_path, settings.MEDIA_ROOT) 
            except Exception as e:
                print(f"ERROR cannot save frame: {e}")

            # Collect detailed error information for the webpage
            error_type = error.get("stage", "Unknown error")  # 'stage' stores the detailed error description
            timestamp = error.get("timestamp", "Unknown timestamp")

            error_frames_info.append({
                "type": error_type,  # This  contains detailed error description like 'feet too tight'
                "timestamp": round(timestamp, 1),
                "frame": frame_path  
            })

        return {
            "total_squats": self.counter,
    ...
    def detect(self, mp_results, image, timestamp) -> None:
        ...
        try:
            ...
            # Convert predicted_class to string to avoid 'numpy.int64' object has no attribute 'split' error
            predicted_class = str(predicted_class)

            if predicted_class == "down" and prediction_probability >= self.PREDICTION_PROB_THRESHOLD:
                if self.current_stage != "down":
                    self.current_stage = "down"
            elif predicted_class == "up" and prediction_probability >= self.PREDICTION_PROB_THRESHOLD:
                if self.current_stage == "down":  # This is where the counter should increment.
                    self.counter += 1
